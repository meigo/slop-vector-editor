...
In `src/__tests__/prefs-route-keys.test.ts`, inside `describe("editActionForKey")` — a new case, and
one line added to the existing tool-key case:
...
- Modify: `src/tools/types.ts`, `src/tools/registry.ts`, `src/state/keys.ts`, `src/state/appState.svelte.ts`, `src/lib/ToolStrip.svelte`
- Test: `src/__tests__/pen-tool.test.ts` (new)
...
**`src/state/keys.ts`:** `TOOL_KEYS` gains `p: "pen",`.
