...
- `moveNodesTo`'s second parameter is renamed `parentId` and passed straight to `moveNodes`; its `layerBlock` guard now applies to the **layer that holds** the target: `const layerId = doc.layers.some((l) => l.id === parentId) ? parentId : findNode(app.doc, parentId)?.layer.id;` — when that layer is blocked, or the target is unknown, return without editing.
