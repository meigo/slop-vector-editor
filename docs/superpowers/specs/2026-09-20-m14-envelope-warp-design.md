# slop-vector-editor — milestone 14 design: envelope warp

Date: 2026-09-20. Status: approved in brainstorming, then **deferred** on 2026-09-21 and renumbered
three times — M11 → M12 → M13 → M14 — as smaller milestones were pulled ahead of it. Nothing below is
...
pulled ahead of it too, on the same reasoning, and Save to Files
(`2026-09-21-m13-save-to-files-design.md`) after that — the last of those not on size but on
urgency: it fixes a save path that is actively broken on a first-class device. Nothing below is withdrawn; it is next after M11.
